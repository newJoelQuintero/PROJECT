        ...
        <div class="body__container">
            <div class="form">
                ...
                <div class="actions__bar">
                    <a href="{{ url('/citas/create') }}" class="btn-primary">+ Nueva Cita</a>
                    <input type="text" id="searchInput" class="search-bar" placeholder="Buscar cita...">
                </div>
                ...
                <div class="table__wrapper">
                    <table class="table" id="citasTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $cita->id_cita }}</td>
                                <td>{{ \Carbon\Carbon::parse($cita->hora_cita)->format('d/m/Y H:i') }}</td>
                                <td>{{ $cita->lugar_cita }}</td>
                                <td>${{ number_format($cita->valor_cita, 2, ',', '.') }}</td>
                                <td>{{ $cita->nombre_paciente }}</td>
                                <td>
                                    <a href="{{ url('/citas/'.$cita->id_cita.'/edit') }}" class="btn-edit">Editar</a>
                                    <form action="{{ url('/citas/'.$cita->id_cita) }}" method="POST" class="inline-form">
                                        @csrf @method('DELETE')
